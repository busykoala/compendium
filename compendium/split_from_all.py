...
section_markers = {
    ...
    "pregnancy_lactation_period": (
        "Schwangerschaft/Stillzeit",
        "Wirkung auf die Fahrtüchtigkeit und auf das Bedienen von Maschinen",
    ),
    "effects_on_the_ability_to_drive_and_operate_machinery": (
        "Wirkung auf die Fahrtüchtigkeit und auf das Bedienen von Maschinen",
...
def parse_medications(medication_texts: List[str]) -> List[Medication]:
    ...
    for med_text in medication_texts:
        ...
        end_index = (
            med_text.find(end_header) + len(end_header)
            if end_header in med_text
            else 0
        )
        ...
        for attr, (start_marker, end_marker) in section_markers.items():
            ...
            end_index = (
                med_text.find(end_marker) if end_marker else len(med_text)
            )
            if start_index != -1:
                section_text = med_text[
